...
#modelacion del grid
    ...
    class Semaforo(Agent): #modelacion de los carritos muito bonitos
        ...
        def step(self): 
            ...
            try: 
                ...
                semFrameAct = []
                semFrameAct.append(tempDict )
                self.model.coloresSemaforos.append(semFrameAct)
    ...
    class car(Agent):
        ...
        def getNextMove(self):
            ...
            try: 
                carrosFrameAct =self.model.posicionesCarros[self.model.frame]
                carrosFrameAct.append([self.unique_id, self.pos] )
                self.model.posicionesCarros[self.model.frame] = carrosFrameAct
            except:#si es el primer carro del frame
                carrosFrameAct = []
                carrosFrameAct.append(tempDict )
                self.model.posicionesCarros.append(carrosFrameAct)
    ...
    salida = model.posicionesCarros

   
    #salida+=model.coloresSemaforos
    #all_grid.to_string()       
    ...
    jsonStr = json.dumps(model.posicionesCarros)
    jsonStr2 = json.dumps(model.coloresSemaforos)

    salida = "carros: " + jsonStr + " semaforos: " + jsonStr2

    #asi era el return de ejemplo
    #return jsonify([{"carros":jsonStr}, {"semaforos": jsonStr2}])
    return(salida)
    #return ( salida  )
