...
# In[1]:
    ...
    #            self.model.roadTurn = 0 
        ...
        """def step(self):
            ...
            if (self.model.carsWaiting0 > most_cars):
                most_cars = self.model.carsWaiting0
            if (self.model.carsWaiting1 > most_cars):
                most_cars = self.model.carsWaiting1
            if (self.model.carsWaiting2 > most_cars):
                most_cars = self.model.carsWaiting2
            if (self.model.carsWaiting3 > most_cars):
                most_cars = self.model.carsWaiting3
            #revisa cual fue elegido para despues ver si se puede ceder el turno al carril (por el cooldown)
            if (self.model.carsWaiting0 == most_cars and self.model.coolDown_time ==0):
                #aqui se cede el turno
                self.model.turno = 3 #prende el semaforo para los carros q van hacia la derecha
                self.model.coolDown_time = 3

            elif (self.model.carsWaiting1 == most_cars and self.model.coolDown_time ==0):
                #aqui se cede el turno
                self.model.turno = 1 #prende el semaforo para los carros q van hacia arriba
                self.model.coolDown_time = 3

            elif (self.model.carsWaiting2 == most_cars and self.model.coolDown_time ==0):
                #aqui se cede el turno
                self.model.turno = 0 #prende el semaforo para los carros q van hacia la izq
                self.model.coolDown_time = 3

            elif (self.model.carsWaiting3 == most_cars and self.model.coolDown_time ==0):
                #aqui se cede el turno
                self.model.turno = 2 #prende el semaforo para los carros q van hacia  abajo
                self.model.coolDown_time = 3
            else:
                self.model.coolDown_time -=1 #reduce el cooldown hasta que llegue a 0 y pueda volver a cambiar turno"""
    ...
    class car(Agent):
        ...
        def getNextMove(self):
            ...
            if self.pos[1] == model.width -2 and self.direction == 1:
        ...
        def step(self) :
            ...
            if (decision == 1 and self.coolDown == False):
                if (randCar == 0 and self.carCount0 < carsAmount):
                    x = 8
                    y = 0
                    b = car((self.frame + 200, self.frame + 2001), 1,(0,0),self)
            ...
            """if (decision == 1):
                if (randCar == 0):
                    self.spawnCar[randCar].unique_id[0] +=10*self.frame
                    self.spawnCar[randCar].unique_id[1] +=10*self.frame
                    self.schedule.add(self.spawnCar[randCar])
                    self.grid.place_agent(self.spawnCar[randCar],(9,0))
                elif (randCar == 1):
                    self.spawnCar[randCar].unique_id[0] +=10*self.frame
                    self.spawnCar[randCar].unique_id[1] +=10*self.frame
                    self.schedule.add(self.spawnCar[randCar])
                    self.grid.place_agent(self.spawnCar[randCar],(14,8))
                elif (randCar == 2):
                    self.spawnCar[randCar].unique_id[0] +=10*self.frame
                    self.spawnCar[randCar].unique_id[1] +=10*self.frame
                    self.schedule.add(self.spawnCar[randCar])
                    self.grid.place_agent(self.spawnCar[randCar],(7,14))
                elif (randCar == 3):
                    self.spawnCar[randCar].unique_id[0] +=10*self.frame
                    self.spawnCar[randCar].unique_id[1] +=10*self.frame
                    self.schedule.add(self.spawnCar[randCar])
                    self.grid.place_agent(self.spawnCar[randCar],(0,6))"""
    ...
    #asi era el return de ejemplo
    #return jsonify([{"message":"Hello World from IBM Cloud!"}])

    return ( all_grid.to_string()  )
